...
type sepEngine struct {
	storage.Engine
	smEng storage.Engine
}

var _ kvstorage.SeparatedEngine = (*sepEngine)(nil)

// SMEngine implements kvserver.SeparatedEngine.
func (e *sepEngine) SMEngine() storage.Engine {
	return e.smEng
}
...
func (cfg *Config) CreateEngines(ctx context.Context) (Engines, error) {
	...
	for i, spec := range cfg.Stores.Specs {
		...
		var smStorageConfigOpts []storage.ConfigOption
		addCfgOpt := func(opt ...storage.ConfigOption) {
			storageConfigOpts = append(storageConfigOpts, opt...)
			smStorageConfigOpts = append(smStorageConfigOpts, opt...)
		}
		addCfgOpt(
			walFailoverConfig,
