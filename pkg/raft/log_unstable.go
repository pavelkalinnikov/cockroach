...
type unstable struct {
	...
	//
	// The invariant enforces the order of handling a situation when there is both
	// a snapshot and entries. The snapshot write must be acknowledged first,
...
func (u *unstable) stableTo(mark logMark) {
	if mark.term < u.term {
		...
		u.logger.Panicf("entry %d acked earlier than the snapshot: %s",
			mark.index, DescribeSnapshot(*u.snapshot))
		return
	}
	u.logSlice = u.forward(mark.index)
	// TODO(pav-kv): why can mark.index overtake u.inProgress? Probably bugs in
