...
func GC(
	...
	if !args.Threshold.IsEmpty() {
		...
		if updated {
			if err := MakeStateLoader(cArgs.EvalCtx).SetGCThreshold(
				ctx, stateloader.SMReadWriter{ReadWriter: readWriter}, cArgs.Stats, &gcThreshold,
			); err != nil {
				return result.Result{}, err
			}
	...
	if len(args.Keys) != 0 || len(args.RangeKeys) != 0 || args.ClearRange != nil {
		...
		hint, err := sl.LoadGCHint(ctx, stateloader.SMReader{Reader: readWriter})
		if err != nil {
			return result.Result{}, err
		}
		if hint.UpdateAfterGC(gcThreshold) {
			...
			if err := sl.SetGCHint(ctx, stateloader.SMReadWriter{ReadWriter: readWriter}, cArgs.Stats, hint); err != nil {
				return result.Result{}, err
			}
		}
	}
