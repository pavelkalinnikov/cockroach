...
func splitTrigger(
	...
	// RHS replica.
	currentStats, err := MakeStateLoader(rec).LoadMVCCStats(ctx, stateloader.SMReader{Reader: batch})
	if err != nil {
		return enginepb.MVCCStats{}, result.Result{}, errors.Wrap(err,
...
func splitTriggerHelper(
	...
	// both sides, we need to update it as well.
	{
		...
		leftLease, err := sl.LoadLease(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
		...
		gcThreshold, err := sl.LoadGCThreshold(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
		...
		gcHint, err := sl.LoadGCHint(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
		...
		// only.
		replicaVersion, err := sl.LoadVersion(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
		...
		*h.AbsPostSplitRight(), err = stateloader.WriteInitialReplicaState(
			ctx, stateloader.SMReadWriter{ReadWriter: batch}, *h.AbsPostSplitRight(), split.RightDesc, rightLease,
...
func mergeTrigger(
...

	{
		...
		lhsHint, err := lhsLoader.LoadGCHint(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
			return result.Result{}, err
		}
		rhsLoader := stateloader.Make(merge.RightDesc.RangeID)
		rhsHint, err := rhsLoader.LoadGCHint(ctx, stateloader.SMReader{Reader: batch})
		if err != nil {
			return result.Result{}, err
		}
		if lhsHint.Merge(rhsHint, rec.GetMVCCStats().HasNoUserData(), merge.RightMVCCStats.HasNoUserData()) {
			if err := lhsLoader.SetGCHint(ctx, stateloader.SMReadWriter{ReadWriter: batch}, ms, lhsHint); err != nil {
				return result.Result{}, err
			}
